    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a professional financial advisor. Respond ONLY in valid JSON."
                },
                {
                    "role": "user",
                    "content": prompt
...
async def voice_transaction(
    ...
    logger.info("Voice audio received")
...
@router.post("/voice-transaction/simulate")
async def simulate_voice_transaction(
    request: VoiceTransactionRequest,
    current_user=Depends(get_current_user)
):
    logger.info(f"Simulating voice transaction: {request.text}")
...
async def confirm_voice_transaction(
    ...
    new_txn = {
        ...
        "created_at": data.date   # ✅ use parsed date
    }
